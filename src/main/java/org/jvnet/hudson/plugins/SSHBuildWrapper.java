...
public final class SSHBuildWrapper extends BuildWrapper {
    ...
    private boolean executePreBuildScript(AbstractBuild<?, ?> build, BuildListener listener)
            throws IOException, InterruptedException {
    ...
    private boolean executePostBuildScript(AbstractBuild<?, ?> build, BuildListener listener)
            throws IOException, InterruptedException {
        PrintStream logger = listener.getLogger();
        CredentialsSSHSite site = getSite();
        if (site == null) {
    ...
    public static final class DescriptorImpl extends BuildWrapperDescriptor {
        ...
        public ListBoxModel doFillCredentialIdItems(final @AncestorInPath ItemGroup<?> context) {
            ...
            return new SSHUserListBoxModel()
                    .withEmptySelection()
                    .withMatching(SSHAuthenticator.matcher(JSchConnector.class), credentials);
        }
        ...
        public FormValidation doLoginCheck(
            ...
            final CredentialsSSHSite site =
                    new CredentialsSSHSite(hostname, port, credentialId, serverAliveInterval, timeout);
            try {
                try {
        ...
        protected XmlFile getConfigFile() {
            ...
            customXstream.addCompatibilityAlias(
                    "org.jvnet.hudson.plugins.SSHSite", CredentialsSSHSite.LegacySSHSite.class);
